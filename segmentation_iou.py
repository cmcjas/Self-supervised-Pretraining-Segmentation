...
def segmentation_iou(batch_size, learning_rate, weight_decay):
    ...
    pets_dataset = torchvision.datasets.OxfordIIITPet(root=pets_path, download=True, transforms=transform, target_types='segmentation')

    develop_size = int(len(pets_dataset) * 0.9) # Test at 10%
    develop_set, test_set = random_split(pets_dataset, [develop_size, int(len(pets_dataset) - develop_size)])
    train_size = int(len(pets_dataset) * 0.8 * 0.9) # Train at 81%, Val at 9%
    ...
    filtered_state_dict = torch.load('resnet18_pretrain_stl10_weights_128_0.0001.pt')
